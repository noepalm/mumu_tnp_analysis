...
void FPCanvasStyle(TVirtualPad* pad, std::string left_text="", std::string right_text="", int iPosX=0, TString extraText="",
                   bool outOfFrame=false, bool is2DCOLZ=false)
{
  ...
  if(iPosX==0)
    ...
    leftText += TString::Format("#scale[0.76]{#bf{ \"%s\"}}", extraText.Data());
  if( outOfFrame )
    {
