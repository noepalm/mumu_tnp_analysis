void draw2Deff(string outpath){
	...
	while(std::getline(fpt, line)){
		...
		Double_t tmp;
		(stringstream)line >> eta_min >> eta_max >> pt_min >> pt_max >> eff >> eff_err >> tmp >> tmp >> tmp >> tmp >> alteff >> alteff_err >> tmp >> tmp;
	...
	TCanvas* c2 = new TCanvas("ceff2d_altsig", "ceff2d_altsig", 1500, 1000);
	c2->Divide(2,1);

	c2->cd(1);
	alt_geta->SetTitle("Efficiency vs #eta;#eta;Efficiency");
	alt_geta->Draw("AP");

	c2->cd(2);
 	alt_gpt->SetTitle("Efficiency vs p_{T};p_{T} [GeV];Efficiency");
	alt_gpt->Draw("AP");
